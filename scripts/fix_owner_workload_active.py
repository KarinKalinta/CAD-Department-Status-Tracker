...
SOURCE = Path("CAD_Status_Tracker_Batch_Aware_MANAGER_VIEW_FINAL_source.xlsx")
...
def main():
    ...
    qty = "CADTracker_Qty"
    plan_end = "CADTracker_Plan_End"

    for row in range(first_owner_row, last_owner_row + 1):
        owner_cell = f"A{row}"
        ws.cell(row, 2).value = f'=COUNTIFS({task},"<>",{owner},{owner_cell},{status},"<>Done",{status},"<>Cancelled")'
        ws.cell(row, 3).value = (
        ...
        ws.cell(row, 4).value = f'=COUNTIFS({task},"<>",{owner},{owner_cell},{plan_end},"<"&TODAY(),{status},"<>Done",{status},"<>Cancelled")'
